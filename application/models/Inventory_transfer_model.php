...
class Inventory_transfer_model extends CI_Model {
	...
	public function search_recipient_code($recipient){
	    $query = $this->db->query("SELECT outlet_code, id, outlet_name, '1' AS recipient_type FROM outlet WHERE comp_id = ? AND id != ? AND outlet_code LIKE ?", array($this->session->userdata("comp_id"), $this->session->userdata("outlet_id"), '%'.$recipient.'%'))->result();
	    return $query;
	}

	public function search_recipient_name($recipient){
	    $query = $this->db->query("SELECT outlet_code, id, outlet_name, '1' AS recipient_type FROM outlet WHERE comp_id = ? AND id != ? AND outlet_name LIKE ?", array($this->session->userdata("comp_id"), $this->session->userdata("outlet_id"), '%'.$recipient.'%'))->result();
	    return $query;
	}
	...
	public function inv_no_wo_id($inv_no){
		$query = $this->db->query("SELECT inv_no FROM inventory_hdr WHERE outlet_id = ? AND inv_type = ? AND inv_no = ?", array($this->session->userdata("outlet_id"), "3", $inv_no))->num_rows();
		return $query;
	}

	public function inv_no_w_id($inv_no, $id){
		$query = $this->db->query("SELECT inv_no FROM inventory_hdr WHERE outlet_id = ? AND inv_type = ? AND inv_no = ? AND id != ?", array($this->session->userdata("outlet_id"), "3", $inv_no, $id))->num_rows();
		return $query;
	}

  	public function select_transfer_no($transfer_no){
  		$query = $this->db->query("SELECT inv_no FROM inventory_hdr WHERE outlet_id = ? AND inv_type = ? AND inv_no = ?", array($this->session->userdata("outlet_id"), "3", $transfer_no))->result();
  		return $query;
  	}

  	public function insert_transfer_hdr($transfer_hdr){
  		$data = array(
  	...
  	public function insert_transfer_dtl($transfer_dtl, $id){
  		$data = array();
  	...
  	public function query_inventory_transfer($keyword,$type,$outlet,$trans_date){
  		$query_keyword = "";
  		$str1= "";
  		...
  		if (!empty($trans_date)){
  			$str1 = "AND `inventory_hdr`.`inv_date` = '".$trans_date."' ";
  		}
		...
		WHERE `inventory_hdr`.`status` = '1' AND `inventory_hdr`.`inv_type` = '3' and `inventory_hdr`.`tran_type` = '".$type."'
		AND outlet_id = '$outlet' ".$query_keyword." ".$str1." 
  	...
  	public function get_inventory_transfer_hdr($trans_id){
  		$query = $this->db->query("
  		SELECT
		    `inventory_hdr`.*
		    , `outlet`.`outlet_code`
  	...
  	public function get_inventory_transfer_dtl($trans_id){
  		$query = $this->db->query("
  		SELECT
  	...
  	public function update_transfer_hdr($transfer_hdr, $id){
  		$data = array(
  			"inv_no" => $transfer_hdr['trans_no'],
  			"inv_date" => $transfer_hdr['trans_date'],
  			"tran_type" => $transfer_hdr['trans_type'],
  	...
  	public function update_transfer_dtl($transfer_dtl){
  	...
  	public function delete_transfer_dtl($id){
  		$this->db->where("id", $id);
  	...
  	public function get_trans_type(){
        $query = $this->db->query("SELECT * FROM inventory_ref_type where inventory_type = '3'")->result();
        return $query;
    }
