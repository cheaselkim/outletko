	...
	function tbl_query($query, $app_func){
		...
		$output .= "<table class='table table-striped table-fixed fixed_header text-table table-hover table-sm' id='tbl-data'>
						<thead class='w-100'>
                            ...
                            <th class='text-left' style='width: 12%;'>Transaction Type</th>
		...
		if (!empty($query)){
			...
			foreach ($query as $key => $value) {
				...
				if ($app_func == "2"){
					...
					$button = "<button class='btn btn-success btn-block' onclick = 'view_transfer(".$value->id.", ".$value->recipient_type.")'>View</button>";
				}else if ($app_func == "4"){
