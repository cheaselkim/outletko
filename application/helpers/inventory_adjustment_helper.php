		...
		$output .= "<table class='table table-striped table-bordered table-sm' id='tbl-query'>
						<thead class='w-100'>
							<tr>
								...
								<th style='width: 20%;' class='font-weight-bold'>Product Nane</th>
								<th style='width: 7%;' class='font-weight-bold'>Product Quality</th>
								<th style='width: 5%;'  class='font-weight-bold'>Unit Price</th>
								<th style='width: 7%;'  class='font-weight-bold'>Stock on Hand</th>
								<th style='width: 3%;'  class='font-weight-bold'>Unit</th>
		...
		if (!empty($query)){
			...
			foreach ($query as $key => $value) {
				...
				$output .= "<tr>
								...
								<td style='width: 20%;' class='product_name'>".$value->product_name."</td>
								<td style='width: 7%;' class='product_grade'>".$prod_grade."</td>
								<td style='width: 5%;' class='product_price text-right'>".number_format($value->reg_selling_price, 2)."</td>
								<td style='width: 7%;' class='product_qty text-right'>".number_format($inv_qty, 2)."</td>
								<td style='width: 3%;' class='product_unit'>".$value->unit_code."</td>
	...
	function query($query){
		...
		foreach ($query as $key => $value) {
			...
			$output .= "<tr>
							...
							<td style='width: 7%;'>".date('m/d/Y', strtotime($value->inv_date))."</td>
							<td style='width: 5%;'>".$value->product_no."</td>
							<td style='width: 20%;'>".$value->product_name."</td>
							<td style='width: 7%;'>".$prod_grade."</td>
							<td style='width: 7%;' class='text-right'>".number_format($value->qty, 2)."</td>
							<td style='width: 3%;'>".$value->unit_code."</td>
						</tr>";
		}
