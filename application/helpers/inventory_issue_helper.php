		...
		$output .= "<table class='table table-striped table-bordered table-hover table-sm' id='tbl-sales-register-dtl'>
					<thead class='w-100'>
						<tr>
							...
							<th class='text-left ' style='width: 3%;'>Unit</th>
		...
		if (!empty($query)){
			foreach ($query as $key => $value) {
				...
				$output .= "<tr class='cursor-pointer'>
								...
								<td style='width :20%;'class='tbl-prod-name bg-button'>".$value->product_name."</td>
								<td style='width :5%;' class='tbl-qty bg-button'>".$value->qty."</td>
								<td style='width :3%;' class='tbl-unit bg-button'>".$value->unit_code."</td>

								<td style='width :7%;' class='tbl-prod-no-replace'></td>
								<td style='width :20%;' class='tbl-prod-name-replace'></td>
								...
								<td style='width :5%;'><button class='btn btn-orange btn-block btn-query px-0 py-0' onclick='replace_product(".$value->id.", ".$key.")'>Replace</button></td>
							</tr>";
			}
		}else{
			$output .= "<tr>
