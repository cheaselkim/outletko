$(document).ready(function(){
  ...
  $("#search_box").autocomplete({
    minLength: 0,
    source: base_url + "Supplier/search_field/",
...
function index(term="",type="",outlet="",status=""){
  var app_func = $("#supplier_func").val();
  var csrf_name = $("input[name=csrf_name]").val();
  $.ajax({
    data: {term:term,type:type,outlet:outlet,status:status, app_func : app_func, csrf_name}, 
    type: "POST", 
    url : base_url +  "Supplier/supplier_list",
    dataType : "JSON",
    success : function(result){
      $("input[name=csrf_name]").val(result.token);
      $("#query-table").html(result.output);
...
function view_supplier(id){
  ...
  $.ajax({
    ...
    url : base_url + "Supplier/select_id",
    type : "POST",
    dataType : "JSON",
    success : function (result){
      $("input[name=csrf_name]").val(result.token);
...
function delete_supplier(id,key){
  var csrf_name = $("input[name=csrf_name]").val();
  var trans_no = $("#tbl-supplier tbody tr:eq("+(key)+")").find("td:eq(0)").text();
  swal({
...
function edit_supplier(id){
  $("body").empty();
  $("body").load(base_url + "menu/edit_menu/4/7/2/"+id);
}