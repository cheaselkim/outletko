$(document).ready(function(){
    ...
    $("#transfer_no").keyup(function(){
        $(this).removeClass("error");
    });

    $("#transfer_date").keyup(function(){
        $(this).removeClass("error");
    });
...
function currency(){
    $.ajax({
        type : "GET",
        dataType : "JSON",
        url : base_url + "inventory_transfer/currency",
        success : function(data){
...
function preview(){
    ...
    var trans_date  = convert_date($("#transfer_date").val());
...
function check_required_fields(){
	...
	if (trans_no == "" || trans_date == "" || recipient_code == "" || recipient_name == "" || purpose == "" || rows == 0){
        ...
        $.ajax({
            url: base_url + "inventory_transfer/inv_no_wo_id/", 
            dataType: "json",
            type: "POST",
