$(document).ready( function() {
    ...
    jQuery(document).on("click", ".sales_row_table", function(){
        ...
        if ($(this).closest("tr").find(".tbl-date-replace").text() == ""){
            var row = $(this).closest("tr").index();
...
function currency(){
    ...
    $.ajax({
        ...
        success : function(data){
            ...
            $("#currency").append("<option value='"+result.id+"'>"+result.curr_code+"</option>");
        }, error : function(err){
...
function session_outlet(){
    ...
    $.ajax({
        ...
        type : "GET",
        dataType : "JSON",
        url : base_url + "inventory_issue/session_outlet",
        success : function(data){
            $("input[name=csrf_name]").val(data.token);
            $("#outlet_rec").val(data.result);
        }, error : function(err){
            console.log(err.responseText);
        }
    })
...
function max_transno(){
    ...
    $.ajax({
        ...
        type : "POST",
        dataType : "JSON",
        url : base_url + "inventory_issue/max_transno",
        success : function(data){
            $("input[name=csrf_name]").val(data.token);
            $("#issue_no").val(data.data);
        }, error : function(err){
            console.log(err.responseText);
        }
    })
...
function edit_item_table(){
    ...
    if (trans_type == "5"){
        ...
        $(table).find(".tbl_qty_onhand").text($.number($("#qty_on_hand").val(),2));
...
function check_required_fields(){
    ...
    var recipient_type = $('#recipient_code').attr('data-type');
    var recipient_name = $('#recipient_name').val();
    ...
    if(jQuery.trim(issue_no).length <= 0 || jQuery.trim(issue_date).length <= 0 || 
        ...
        var csrf_name = $("input[name=csrf_name]").val();
        $.ajax({
            data : {"issue_no" : issue_no, csrf_name : csrf_name},
            type : "POST",
            dataType : "JSON",
            url : base_url + "inventory_issue/inv_no_wo_id",
            success : function(result){
                $("input[name=csrf_name]").val(result.token);
...
function data_saving(){
      ...
      $.ajax({
            data : data
            , type: "POST"
            , url: base_url + "inventory_issue/save_data"
            , dataType: 'json'
            , crossOrigin: false     
            , beforeSend : function(){
              swal({
